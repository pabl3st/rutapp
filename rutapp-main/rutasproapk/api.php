...
if ($action === 'delta_sync') {
    ...
    // KPI values en stops actualizados
    $stK = db()->prepare(
        ...
        foreach ($ops as $op) {
            ...
            try {
                if ($entity === 'route') {
                    ...
                    // Upsert valores KPI de una visita (stop)
                    // data = { stopUid, values: {kpiId: value, ...} }
                    if ($operation === 'upsert' || $operation === 'create' || $operation === 'update') {
                    ...
                    // Upsert perfil de negocio (sector)
                    if ($operation === 'upsert' || $operation === 'create' || $operation === 'update') {
                        db()->prepare(
